...
class FacebookBrowserClient(FacebookClient):
    ...
    DEFAULT_PROFILE_DIR = (
        r"D:\facebook_analyzer\data\facebook_chrome_profile"
    )

    DEFAULT_WAIT_MS = 2500

    # Streamlit rerun tạo FacebookBrowserClient mới. Giữ Group ID cuối
    # ở cấp class để client mới vẫn biết Group nào đang được phân tích.
    # Điều này đặc biệt quan trọng khi nút "Phân tích bình luận" tạo
    # một Playwright client mới sau khi lượt quét Group đã kết thúc.
    ...
    def __init__(
        ...
        # Khôi phục Group ID từ client trước trong cùng tiến trình Streamlit.
        self._last_group_id: str = self.__class__._global_last_group_id
    ...
    def _has_facebook_session(self) -> bool:
        ...
        try:
            ...
            if any(cookie.get("name") == "c_user" and cookie.get("value") for cookie in cookies):
                return True
        except Exception:
            pass
        try:
    ...
    def ensure_login(self) -> bool:
        ...
        print()
        print("=" * 70)
        print("CHƯA PHÁT HIỆN PHIÊN ĐĂNG NHẬP FACEBOOK")
        print("=" * 70)
        print("Chrome đã được mở bằng profile riêng.")
        print("Hãy đăng nhập Facebook THỦ CÔNG trong cửa sổ Chrome.")
        print("Hệ thống sẽ tự kiểm tra phiên đăng nhập trong 60 giây.")
        print("=" * 70)
        try:
        ...
        for remaining in range(60, 0, -1):
            if self._has_facebook_session():
                print("[+] Đã phát hiện Facebook session.")
                return True
            if remaining % 5 == 0 or remaining <= 2:
                print(f"[Login] Chưa đăng nhập, còn khoảng {remaining}s...")
            try:
    ...
    def _get_html(self) -> str:
        """
        Lấy HTML hiện tại của Facebook page.

        Facebook là SPA nên trong lúc chuyển trang/render GraphQL,
        page.content() đôi khi có thể lỗi hoặc page đang ở trạng thái
        chuyển tiếp. Vì vậy dùng nhiều fallback và không nuốt exception.
        """
        page = self._ensure_browser()

        # 1) Playwright page.content()
        try:
        ...
        # 2) Lấy trực tiếp outerHTML từ browser context.
        try:
        ...
        # 3) Lấy outerHTML thông qua locator.
        try:
        ...
        # 4) In trạng thái thật để không còn trường hợp HTML=0
        # nhưng không biết nguyên nhân.
        try:
            print(
                f"[Debug Page] closed={page.is_closed()} "
                f"url={page.url}"
            )
            try:
                print(
    ...
    def _collect_group_posts(
        ...
        self._last_group_id = str(actual_group_id)
        self.__class__._global_last_group_id = str(actual_group_id)
        print(
            f"[Debug] Lưu Group ID cho các lần rerun: {actual_group_id}"
        )
    ...
    def get_post(
        ...
        # Group ID không thể suy ra chắc chắn chỉ từ post ID.
        # Trường hợp này dùng URL hiện tại nếu đang ở post.
        page = self._ensure_browser()

        # Nếu current URL đã có group ID.
        current_url = ""
        ...
        if not group_id.isdigit():
            print(
                "[!] get_post() cần URL post "
                "có Group ID hoặc gọi sau get_group_posts()."
            )
    ...
    @staticmethod
    def _find_nearest_comment_node_object(
        source: str,
        marker_pos: int,
    ) -> dict[str, Any] | None:
        """
        Tìm object node gần legacy_fbid mà không phụ thuộc spacing.

        Facebook có thể sinh:
            {"node":{...}}
            {"node": {...}}
        và trong một số payload JSON nhúng có thể xuất hiện quote escaped.
        """
        window_start = max(0, marker_pos - 30000)
        prefix = source[window_start:marker_pos]

        # Dạng JSON đã decode HTML.
        matches = list(
            re.finditer(
                r'"node"\s*:\s*\{',
                prefix,
        ...
        if matches:
            brace = window_start + matches[-1].end() - 1
            obj = FacebookBrowserClient._extract_balanced_object(
                source,
                brace,
            )
            if isinstance(obj, dict):
                if isinstance(obj.get("node"), dict):
                    return obj
                if "legacy_fbid" in obj:
                    return {"node": obj}

        # Dạng quote escaped: \"node\":{...}
        ...
        if escaped_matches:
            brace = window_start + escaped_matches[-1].end() - 1
            obj = FacebookBrowserClient._extract_balanced_object(
                source,
                brace,
            )
    ...
    def _extract_comment_nodes(
        ...
        # Facebook thay đổi spacing/escaping của JSON khá thường xuyên.
        # Không phụ thuộc chính xác vào dạng `"legacy_fbid":"123"`.
        ...
        for match in legacy_pattern.finditer(source):
            ...
            node_obj = self._find_nearest_comment_node_object(
                source,
                match.start(),
            )
    ...
    def _select_all_comments_filter(self) -> int:
        """Cố gắng chuyển bộ lọc sang 'Tất cả bình luận'."""
        page = self._ensure_browser()
        clicked = 0
        filter_patterns = [re.compile(r"most\s+relevant", re.I), re.compile(r"phù\s+hợp\s+nhất", re.I)]
        all_patterns = [re.compile(r"all\s+comments?", re.I), re.compile(r"tất\s+cả(?:\s+các)?\s+bình\s+luận", re.I)]
        for selector in ('[role="button"]', 'button', 'a[role="button"]'):
            try:
                locator = page.locator(selector); count = min(locator.count(), 180)
            except Exception: continue
            for i in range(count):
                try:
                    el=locator.nth(i)
                    if not el.is_visible(): continue
                    label=self._clean_text(el.get_attribute("aria-label") or el.inner_text() or "")
                    if not label or not any(p.search(label) for p in filter_patterns): continue
                    el.scroll_into_view_if_needed(timeout=1500); el.click(timeout=2500); page.wait_for_timeout(800)
                    clicked += 1; break
                except Exception: continue
            if clicked: break
        selectors=('[role="menuitem"]','[role="option"]','[role="button"]','button','div[tabindex="0"]')
        for selector in selectors:
            try:
                locator=page.locator(selector); count=min(locator.count(),220)
            except Exception: continue
            for i in range(count):
                try:
                    el=locator.nth(i)
                    if not el.is_visible(): continue
                    label=self._clean_text(el.get_attribute("aria-label") or el.inner_text() or "")
                    if not label or not any(p.search(label) for p in all_patterns): continue
                    el.scroll_into_view_if_needed(timeout=1500); el.click(timeout=2500); page.wait_for_timeout(1800)
                    return clicked+1
                except Exception: continue
        for pattern in all_patterns:
            try:
                locator=page.get_by_text(pattern, exact=False); count=min(locator.count(),60)
            except Exception: continue
            for i in range(count):
                try:
                    el=locator.nth(i)
    ...
    def _click_comment_expansion_controls(self) -> int:
        """Click các control tải thêm bình luận cấp cao nhất."""
        page=self._ensure_browser()
        patterns=[
            re.compile(r"view\s+more\s+comments?",re.I), re.compile(r"show\s+more\s+comments?",re.I),
            re.compile(r"more\s+comments?",re.I), re.compile(r"view\s+previous\s+comments?",re.I),
            re.compile(r"show\s+previous\s+comments?",re.I),
            re.compile(r"xem\s+thêm\s+(?:các?\s+)?bình\s+luận",re.I),
            re.compile(r"xem\s+(?:các?\s+)?bình\s+luận\s+trước",re.I),
            re.compile(r"xem\s+thêm\s+bình\s+luận\s+khác",re.I),
        ]
        clicked=0; clicked_keys=set()
        selectors=('[role="button"]','button','a[role="button"]','[role="link"]','div[tabindex="0"]')
        for selector in selectors:
            try: locator=page.locator(selector); count=min(locator.count(),250)
            except Exception: continue
            for i in range(count):
                try:
                    el=locator.nth(i)
                    if not el.is_visible(): continue
                    label=self._clean_text(el.get_attribute("aria-label") or el.inner_text() or ""); low=label.lower()
                    if not label or low in clicked_keys or not any(p.search(label) for p in patterns): continue
                    el.scroll_into_view_if_needed(timeout=1500); el.click(timeout=2500)
                    clicked += 1; clicked_keys.add(low); page.wait_for_timeout(1200)
                except Exception: continue
        for pattern in patterns:
            try: locator=page.get_by_text(pattern,exact=False); count=min(locator.count(),60)
            except Exception: continue
            for i in range(count):
                try:
                    el=locator.nth(i)
                    if not el.is_visible(): continue
                    label=self._clean_text(el.inner_text() or el.get_attribute("aria-label") or ""); low=label.lower()
                    if not label or low in clicked_keys or not any(p.search(label) for p in patterns): continue
                    el.scroll_into_view_if_needed(timeout=1500); el.click(timeout=2500)
                    clicked += 1; clicked_keys.add(low); page.wait_for_timeout(1200)
                except Exception: continue
    ...
    def _scroll_comments_once(self):
        page=self._ensure_browser()
        try: page.mouse.wheel(0,1800)
        except Exception: pass
        try:
            page.evaluate('() => window.scrollBy({top: Math.floor(window.innerHeight * 1.5), left: 0, behavior: "instant"})')
        except Exception: pass
        self._wait_page(1600)
    ...
    def _collect_post_comments(
        ...
        # Ưu tiên Group ID trong URL hiện tại. Nếu client mới được tạo
        # do Streamlit rerun, URL thường chưa phải trang Group nên dùng
        # Group ID đã lưu ở cấp instance/class từ lượt quét trước.
        if not group_id.isdigit():
            group_id = str(self._last_group_id or "")

        if not group_id.isdigit():
            group_id = str(self.__class__._global_last_group_id or "")

        if group_id.isdigit():
            self._last_group_id = group_id
            self.__class__._global_last_group_id = group_id
            print(f"[Debug] Group ID cho comments: {group_id}")
        ...
        if str(post_id) not in current_url:
            print(
                f"[+] Mở trang post để lấy comments: {post_url}"
            )
            ...
            try:
                ...
                print(
                    f"[!] page.goto() exception: "
                    f"{type(exc).__name__}: {exc}"
                )

            # Facebook tiếp tục render GraphQL sau DOMContentLoaded.
            self._wait_page(5000)

            try:
                filter_clicked = self._select_all_comments_filter()
                print(f"[Comment Filter] Clicked: {filter_clicked}")
            except Exception as exc:
                print(f"[!] Không đổi được bộ lọc comment: {type(exc).__name__}: {exc}")

            try:
                print(
                    f"[Debug Post Page] closed={page.is_closed()}"
                )
                print(
                    f"[Debug Post Page] url={page.url}"
                )
                print(
                    f"[Debug Post Page] title={page.title()}"
                )
            except Exception as exc:
                print(
                    f"[!] Không kiểm tra được post page: "
                    f"{type(exc).__name__}: {exc}"
                )
        ...
        for round_index in range(
            ...
            if round_index == 1:
                # Snapshot và thống kê marker để phân biệt:
                # page không có HTML hay parser không nhận dạng được JSON.
                self._save_debug_html(
                    html_text,
                    "comments_round_001.html",
                )

                debug_source = self._decode_possible_html_json(
                    html_text
                )
                legacy_count = len(
                    re.findall(
                ...
                print(
                    f"[Debug Comments] HTML={len(html_text):,} chars | "
                    f"legacy_fbid={legacy_count} | node={node_count} | "
                    f"comment_id={comment_url_count}"
                )
